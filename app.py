...
# Page configuration
st.set_page_config(page_title="NutriBench", layout="wide", page_icon="🥗")
...
with st.sidebar:
    ...
    if not st.session_state.authenticated:
        ...
        st.info(
            "User preferences and settings will be available here in future updates."
        )
...
if st.session_state.authenticated:
        ...
        with col2:
            ...
            if not st.session_state.current_session_id:
                ...
                if current_messages:
                    ...
                    st.info(
                        f"No messages found for session: {st.session_state.current_session_id}"
                    )
    ...
    with tab2:
        ...
        with col1:
            ...
            if uploaded_file is not None:
                st.image(
                    uploaded_file, caption="Uploaded meal photo", use_column_width=True
                )
        ...
        with col2:
            ...
            weight = st.number_input(
                "Weight (kg)", min_value=1.0, max_value=300.0, value=70.0, step=0.1
            )
            ...
            diabetes_risk = st.selectbox(
                "Diabetes Risk Assessment", ["Low", "Medium", "High", "Unknown"]
            )
            heart_disease_risk = st.selectbox(
                "Heart Disease Risk", ["Low", "Medium", "High", "Unknown"]
            )
        ...
        if st.button("🔍 Analyze Meal", type="primary"):
            if meal_description:
                with st.spinner("Analyzing your meal..."):
                    ...
                    st.info(
                        "🍽️ **Estimated Nutrition:** 500 kcal, 30g protein, 12g fat, 45g carbs, 8g fiber"
                    )
                    st.success(
                        "✅ **Recommended Diet:** Low Carb based on your profile"
                    )
    ...
    with tab4:
        ...
        with col2:
            ...
            if st.button(
                "📄 Download Nutrition Report (Coming soon!)",
                type="primary",
                disabled=True,
            ):
